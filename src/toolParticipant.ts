...
export function registerToolUserChatParticipant(context: vscode.ExtensionContext) {
    const handler: vscode.ChatRequestHandler = async (request: vscode.ChatRequest, chatContext: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken) => {
        if (request.command === 'list') {
            stream.markdown(`Available tools: \n\n${vscode.lm.tools.map(tool => tool.name).join(', \n\n')}\n\n`);
            return;
        }
        ...
        const tools =
            vscode.lm.tools.filter(tool => !tool.name.startsWith("vscode_editFile"));
    ...
    const toolUser = vscode.chat.createChatParticipant('migration-agent', handler);
